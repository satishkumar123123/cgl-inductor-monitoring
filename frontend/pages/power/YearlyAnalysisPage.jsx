...
import { fetchYearlyAnalysis } from "../../services/powerService.js";
...
export default function YearlyAnalysisPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const result = await fetchYearlyAnalysis(year);
      setData(result);
    } catch (err) {
      notify(err.response?.data?.message || "Failed to load yearly report", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const log = (format) => logReport({ reportType: "Yearly Analysis", periodLabel: String(year), format }).catch(() => {});

  const downloadPdf = async () => {
    await exportElementToPdf("yearly-report-content", `Yearly_Report_${year}.pdf`);
    await log("pdf");
  ...
  const downloadExcel = () => {
    ...
    const header = ["Month", "Total Power", "Total Production", "Total Dross", "Power/Ton", "Dross %"];
    const body = data.monthly.map((m) => [m.month, m.totalPower, m.totalProduction, m.totalDross, m.powerPerTon, m.drossPercent]);
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([header, ...body]), "Monthly Comparison");
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-lg font-extrabold text-white">Yearly Report</h1>
        <p className="text-xs text-slate-500 mt-1">Full-year totals, averages, and month-by-month comparison, straight from MongoDB Atlas.</p>
      </div>
      ...
      <div className="no-print flex flex-wrap items-center gap-2.5 bg-slate-900/60 border border-slate-800 rounded-2xl p-3.5 backdrop-blur-md">
        <input type="number" value={year} onChange={(e) => setYear(Number(e.target.value))} className="select-input w-28" />
      ...
      {loading ? (
        <div className="flex flex-col gap-4">
          <LoadingSpinner label="Loading yearly report…" />
        ...
        <div className="text-sm text-slate-500 bg-slate-900/60 border border-slate-800 rounded-2xl p-8 text-center">
          No power consumption data saved in MongoDB for {year}.
        </div>
      ) : (
        <div id="yearly-report-content" className="bg-slate-950 border border-slate-800 rounded-2xl p-6 flex flex-col gap-5">
          <h2 className="text-center text-cyan-300 font-extrabold tracking-wide">YEARLY REPORT — {year}</h2>

          <div className="grid gap-3" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))" }}>
            <StatCard icon={Factory} label="Total Production" value={data.summary.totalProduction.toFixed(0) + " Ton"} accent="text-blue-400" />
            <StatCard icon={Zap} label="Total Power" value={data.summary.totalPower.toFixed(0) + " kW"} accent="text-orange-400" />
          ...
          <div className="grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(340px, 1fr))" }}>
            <ChartCard title="Monthly Power (Bar)">
              <BarChart data={data.monthly}>
                <CartesianGrid stroke={chartTheme.grid} vertical={false} />
                <XAxis dataKey="month" tick={chartTheme.tick} /><YAxis tick={chartTheme.tick} /><Tooltip {...chartTheme.tooltip} />
                <Bar dataKey="totalPower" name="Total Power (kW)" fill="#F97316" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ChartCard>
            <ChartCard title="Monthly Trend (Line)">
              <LineChart data={data.monthly}>
                ...
                <Line type="monotone" dataKey="totalPower" name="Power (kW)" stroke="#F97316" strokeWidth={2.5} dot={{ r: 3 }} />
                <Line type="monotone" dataKey="totalProduction" name="Production (Ton)" stroke="#3B82F6" strokeWidth={2.5} dot={{ r: 3 }} />
              </LineChart>
            </ChartCard>
            <ChartCard title="Power/Ton by Month">
              <LineChart data={data.monthly}>
                <CartesianGrid stroke={chartTheme.grid} vertical={false} />
                <XAxis dataKey="month" tick={chartTheme.tick} /><YAxis tick={chartTheme.tick} /><Tooltip {...chartTheme.tooltip} />
            ...
            <ChartCard title="Dross % by Month">
              <BarChart data={data.monthly}>
                <CartesianGrid stroke={chartTheme.grid} vertical={false} />
                <XAxis dataKey="month" tick={chartTheme.tick} /><YAxis tick={chartTheme.tick} /><Tooltip {...chartTheme.tooltip} />
